...
# Input is in the format "region;latitude;longitude"
...
for region in regions:
	...
	#
	c.execute('''SELECT * FROM pricelineregions WHERE regionname="%s"'''	% vars[0])
	region = c.fetchone()
	...
	#	regionid = region[0]
	if not c.execute('''SELECT * FROM pricelineregions WHERE regionid=%s''' % ):
		# 4b Inserts the new region and gets its regionid for use with the points
		#
		c.execute("INSERT INTO pricelineregions (regionname, cities_cityid, latitude, longitude, active) VALUES ('%s','%s','%s','%s','%s')" % (vars[0], vars[1], vars[2], vars[3], 1))
		c.execute("SELECT pricelineregionid FROM pricelineregions WHERE regionname='%s' AND active='1'" % vars[0])
		regionid = c.fetchone()
		regionid = regionid[0]
		open("output/cityadditions.txt", "a").write("%s;%s;%s;%s\n" % (regionid, vars[0], vars[2], vars[3]))
	
	
	# 5  Check if there's a new set of points or not, if so insert the new ones
	#    Compare each point in order, and compare the lengths
	#
	c.execute("SELECT * FROM pricelinepoints WHERE pricelineregions_pricelineregionid = '%s'" % regionid)
	points = c.fetchall()
	diff = False
	...
	if diff == True:
		...
		output = ""
		for line in lines:
			c.execute("INSERT INTO pricelinepoints (pricelineregions_pricelineregionid, orderid, latitude, longitude) VALUES ('%s','%s','%s','%s')" % (regionid, order, line[2], line[3]))
			order += 1
