...
class BBHotwirePostSpider(BaseSpider):
	...
	def parse(self, response):
		...
		regions = hxs.select("//table[@class='ipb_table' and(contains(@summary, 'United'))]/tr")

		for region in regions:

			# 2  Find the board URL's from the current XPaths
			#
			url = region.select(".//h4/a/@href")
			if (url):
...
#		post = hxs.select("//div[@class='postcolor']")
		...
		if (post):
			...
			states = response.request.meta['states']
			open("bbhw.log", "a").write("SAVING HOTEL LIST: `%s` , `%s`\n" % (str(states), response.url)) 
			output = CSV.pack(CSV(), output, "~")
			output = CSV.pack(CSV(), [output, states, "hotwire", "bb"], "@")
