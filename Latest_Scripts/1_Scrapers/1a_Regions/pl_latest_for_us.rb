...
def findCities(browser)
	...
	browser.text_field(:id, "bid_htl_checkin_CTL").set((Time.now + 86400 * 2).strftime("%m/%d/%y"))
	browser.text_field(:id, "bid_htl_checkout_CTL").set((Time.now + 86400 * 3).strftime("%m/%d/%y"))
		...
		html.each_with_index do |place, index|
			...
			#
				...
				if state
					...
					if true then #missing.include?("#{city}, #{state}") then
						begin
#							city_id = find_city(city, state)
							...
							process_page(browser, exec, "#{city}, #{state}")
	...
	matches.each_with_index do |match, index|
		...
		#
			...
			if (region and regionid and lat and lng and points) then
				...
				#
				output += "##{region};#{regionid};#{lat};#{lng}$"
				output += points
