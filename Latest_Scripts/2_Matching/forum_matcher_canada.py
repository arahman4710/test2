...
from processed_forum_data6 import ProcessedRawForumData_canada
from priceline_region_hotel_canada_map import PricelineRegionHotelMap_canada
from unmatched_forum_hotel_canada_table import UnmatchedForumHotelTable_canada
from possible_forum_hotel_canada_match import PossibleForumHotelMatchTable_canada
from matched_forum_hotel_canada_table import MatchedForumHotel_canada
...
def find_state_by_city_id(city_id):
    ...
    #helper function for get_city_regions_from_area function, retrives the state of a given city id
     ...
     return session.query(CityTable_canadian).filter(CityTable_canadian.uid == city_id).one().state
...
def get_city_regions_from_area(area_id):
    ...
    priceline_area_city_table = session.query(PricelineAreaCityTable_canada).filter(PricelineAreaCityTable_canada.priceline_area_id == area_id)
...
def pl_area():
    ...
    priceline_areas = session.query(PricelineAreaTable_canada)
...
def find_cities_not_in_pl():
    ...
    city_ids = session.query(PricelineCityRegionMap_canada.city_id).distinct().union(session.query(distinct(PricelineAreaCityTable_canada.city_id)))

    for city_id in city_ids:

        pl_city_list.append(str(city_id[0]))

    city_ids_from_cities = session.query(CityTable_canadian.uid)
...
def state_city(area_dictionary,excluded_city_id,state_region):
    ...
    relevant_city_list = session.query(CityTable_canadian).filter(~ CityTable_canadian.uid.in_( exclude_city_list ))
