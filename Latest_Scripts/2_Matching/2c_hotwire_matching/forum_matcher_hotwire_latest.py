...
with open('us_state_code','r') as f:
	...
	state_code_dict_reverse[str('District of Columbia')] = str('DC')

def city_region_finder(hotwire_city, hotwire_state):

	hotwire_state_code = state_code_dict_reverse[hotwire_state]
	
	regions = session.query(Neighborhood).filter(Neighborhood.state == hotwire_state_code).filter(Neighborhood.city == hotwire_city)
	region_dict = {}
	if regions.count() > 0:
