...
class PossibleForumHotelMatchTable(Base):
    ...
    __tablename__ = 'possible_forum_hotel_match_hotwire_table'

    uid = Column(Integer, Sequence('possible_forum_hotel_match_sequence'), primary_key=True)   #   primary key
    unmatched_entry_id = Column(Integer, ForeignKey('unmatched_forum_hotel_hotwire_table.uid')) #   primary key for the umatched forum hotel entry that the record is related to
    hotel_id = Column(Integer())    #   primary key of an internal hotel that is considered to be a possible match for the unmatched forum hotel in question
