...
for Do in toDo:
	...
	# 3a  Delete all old points for the current city, error if no city is found in priceline regions
	#
	city = (re.split(";", points[0]))[0]
	c.execute("SELECT pricelineregionid FROM pricelineregions WHERE regionname = '%s'" % city)
	cityid = c.fetchone()
	if cityid:
		cityid = cityid[0]
		c.execute("DELETE FROM pricelinepoints WHERE pricelineregions_pricelineregionid = '%s'" % cityid)
		input = re.sub("\s*%s\s*" % lines[Do - 1], "", input)
	else:
		raise Exception("The city corresponding to those points cannot be found in the table")
	
	
	# 3b  Insert all the new points into the database
	#
	for point in points:
		point = re.split(";", point)
		c.execute("INSERT INTO pricelinepoints (pricelineregions_pricelineregionid, orderid, latitude, longitude) VALUES ('%s','%s','%s','%s')" % (cityid, order, point[1], point[2]))
		order += 1
