...
def make_region(region_name,points):
    point_set = []
    edge_set = []

    for point in points:
        point_set.append(Pt(x=point[0], y=point[1]))

    if point_set[0]!=point_set[-1]:
        point_set.append(point_set[0])

    for i in range(len(point_set)-1):
        edge_set.append(Edge(point_set[i],point_set[i+1]))

    return Poly(name= region_name, edges=edge_set)
