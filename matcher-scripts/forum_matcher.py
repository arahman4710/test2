...
from priceline_region_hotel_map import PricelineRegionHotelMap
from matched_forum_hotel_table import MatchedForumHotel
...
def city_region_finder(cityid):
    ...
    region_ids = session.query(PricelineCityRegionMap).filter(PricelineCityRegionMap.city_id == int(cityid)).all()
    ...
    for region_id in priceline_region_ids:
        ...
        priceline_region_record = session.query(PricelineRegionTable).filter(PricelineRegionTable.priceline_id == region_id).all()
...
def find_state_by_city_id(city_id):
    ...
    #helper function for get_city_regions_from_area function, retrives the state of a given city id
     ...
     return session.query(CityTable).filter(CityTable.uid == city_id).one().state
...
def get_city_regions_from_area(area_id):
    ...
    priceline_area_city_table = session.query(PricelineAreaCityTable).filter(PricelineAreaCityTable.priceline_area_id == area_id)

    #2  iterate through all the cities in the area

    for record in priceline_area_city_table:
...
def pl_area():
    ...
    priceline_areas = session.query(PricelineAreaTable)

    #2  iterate through all the area

    for area in priceline_areas:

        #for each area, get all the associated cities

        area_id = area.uid
        area_name = str(re.sub(', [A-Za-z][A-Za-z]', '', area.name)).lower()
...
def find_cities_not_in_pl():
    ...
    city_ids = session.query(PricelineCityRegionMap.city_id).distinct().union(session.query(distinct(PricelineAreaCityTable.city_id)))

    for city_id in city_ids:

        pl_city_list.append(str(city_id[0]))

    city_ids_from_cities = session.query(CityTable.uid)

    for city_id in city_ids_from_cities:
...
def state_city(area_dictionary,excluded_city_id,state_region):
    ...
    exclude_city_list = tuple(excluded_city_id)

    #cursor.execute("Select * from cities where cityid not in " + exclude_city_list)
    relevant_city_list = session.query(CityTable).filter(~ CityTable.uid.in_( exclude_city_list ))

    for city_obj in relevant_city_list:
        ...
        if city_obj.state.lower() in state_city.keys():
            ...
            if city_obj.name.lower() not in state_city[city_obj.state.lower()].keys():
                ...
                state_city[city_obj.state.lower()][city_obj.name.lower()] = city_region_finder(city_obj.uid)

                area_city[city_obj.uid] = str(city_obj.state)+'|'+str(city_obj.name)
                #c  store city id for later processing hashing 'state_name|city_name' key

        else:

            state_city[city_obj.state.lower()] = {}
            state_city[city_obj.state.lower()][city_obj.name.lower()] = city_region_finder(city_obj.uid)
            area_city[city_obj.uid] = str(city_obj.state)+'|'+str(city_obj.name)

        if city_obj.state.lower() not in state_region.keys():

            state_region[city_obj.state.lower()] = {}

        state_region[city_obj.state.lower()].update(state_city[city_obj.state.lower()][city_obj.name.lower()].items())
...
def retrive_raw_scraped_from_db():
    ...
    return session.query(ProcessedRawForumData)
...
def region_hotel_finder(region_id):
    ...
    hotel_ids = session.query(PricelineRegionHotelMap.hotel_id).filter(PricelineRegionHotelMap.priceline_region_id == region_id )

    #   extract hotel_ids from query
    
    hotel_ids = tuple(map(lambda x: x[0],hotel_ids))

    region_name = session.query(PricelineRegionTable.name).filter(PricelineRegionTable.uid == region_id).one()[0]
...
def insert_matched_forum_hotel_entries(db_record):
    ...
    (forum_hotel_id,internal_hotel_id) = db_record

    matched_forum_hotel_record = MatchedForumHotel(forum_hotel_id,internal_hotel_id)
...
def get_area_dict(target_site):
    ...
    priceline_area_records = session.query(PricelineAreaTable.uid, PricelineAreaTable.name)
...
def get_city_dict(target_site):
    ...
    exclude_list = str(tuple(find_cities_not_in_pl()))

    get_city_lst = session.query(CityTable.uid,CityTable.name,CityTable.state).filter(~CityTable.uid.in_(exclude_list))
...
def test_matcher():
    ...
    test_log = open('hotel_matches.csv','w')
